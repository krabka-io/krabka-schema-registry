...
**Packaging.** The Helm chart is in this repository, at [`charts/krabka-schema-registry`](../charts/krabka-schema-registry). The `SchemaRegistry` CRD is in [`krabka-io/krabka-operator`](https://github.com/krabka-io/krabka-operator), under the `krabka.io` API group, because the CRD belongs to the operator crate. The apko image definition is in [`krabka-io/tooling`](https://github.com/krabka-io/tooling) with the other shared base-image inputs. M7 states where those two live and links to them. It does not copy them here.
